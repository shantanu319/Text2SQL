...
def tokenize_function(examples, tokenizer, max_length=512):
    ...
    Returns lists rather than tensors so that tensor conversion happens on the fly.
    """
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token

    prompts = []
    # Build prompts in one go using list comprehension.
    ...
    # For labels, simply copy the input_ids list
...
def prepare_sql_dataset(file_path, tokenizer, test_size=0.1, max_length=512):
    ...
    # Convert datasets to torch format on the fly during training
...
def load_spider_dataset(spider_dir, tokenizer, max_length=512, test_size=0.4):
    ...
    # Use sklearn's train_test_split then convert back to a Dataset
...
def finetune_model(model_name, train_dataset, eval_dataset, output_dir="./fine_tuned_model", 
    ...
    # Enable offloading to CPU to reduce GPU memory usage.
    model = AutoModelForCausalLM.from_pretrained(
        ...
        offload_state_dict=True             # Offload model states to CPU
    )
